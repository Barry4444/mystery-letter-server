...
const io = new Server(server, {
  path: '/socket.io',
  cors: {
    origin: process.env.ALLOWED_ORIGIN || '*', // zet hier je Netlify domein voor striktere CORS
    methods: ['GET', 'POST'],
    credentials: true,
  },
});
...
const socketToRoom = new Map(); // socketId -> roomId  (betrouwbare cleanup)
...
function broadcastState(room) {
  ...
  // eigen handen naar eigen socket
...
function ensureOrder(room) {
  // Bots DOEN mee in de beurtvolgorde (anders spelen ze nooit)
  room.order = [...room.players.keys()].filter(
    id => room.players.get(id).alive !== false
  );
  if (room.order.length && !room.turn) {
    // start bij eerste in de lijst
    const nextId = room.order[0];
    // bescherming eindigt wanneer je eigen beurt start
    const nextP = room.players.get(nextId);
    if (nextP) nextP.protected = false;
    room.turn = nextId;
  }
}
...
function nextTurn(room) {
  ...
  // bescherming vervalt aan het begin van je beurt
  const nextP = room.players.get(nextId);
  if (nextP) nextP.protected = false;
  room.turn = nextId;
}
...
/* Wincondition per ronde: laatste levende krijgt 1 munt, 3 munten = spelwinnaar */
...
io.on('connection', (socket) => {
  ...
  socket.on('game:startRound', (_p, ack) => {
    ...
    room.turn = null; // reset — laat ensureOrder de eerste bepalen
    ensureOrder(room);
  ...
  socket.on('game:play', (payload, ack) => {
    ...
    const me = room.players.get(socket.id);
    if (!me || !Array.isArray(me.hand) || me.hand.length !== 2) {
      return ack?.({ ok: false, error: 'need 2 cards in hand' });
    }
    ...
    // Speciale regel: Heks + Prinses samen -> Prinses MOET af, speler verliest
    if (card.key === 'heks' && other?.key === 'prinses') {
      ...
      if (maybeEndRound(room)) { ack?.({ ok: true }); return; }
      nextTurn(room);
      broadcastState(room);
      ack?.({ ok: true });
      maybeBotTurn(room);
    ...
    if (needsTarget) {
      // ridder mag altijd (ook jezelf en beschermden)
      ...
      if (!validTarget) {
        if (allowNoTarget && card.key !== 'prinses') {
          // kaart zonder effect afleggen om soft-lock te voorkomen
    ...
    if (card.key === 'ziener') {
      ...
      if (!target || !target.alive || target.id === socket.id) {
        return ack?.({ ok: false, error: 'valid target required' });
      }
      const mine = other;               // mijn NIET-gespeelde kaart
      const theirs = target.hand?.[0];  // target’s kaart
      if (mine && theirs) {
        // symmetrische swap — let op de index van mijn "andere" kaart
        target.hand[0] = mine;
      ...
      // solo Heks: geen direct effect; de speciale combinatie staat bovenaan
  ...
  socket.on('admin:kick', ({ playerId }, ack) => {
    ...
    if (targetSocket) {
      ...
      // harde disconnect; cleanup via 'disconnecting'
      targetSocket.disconnect(true);
    } else {
      // offline socket -> handmatig opruimen
...
function maybeBotTurn(room) {
  ...
  setTimeout(() => {
    ...
    // kies willekeurig één kaart om af te leggen (geen effecten uitgewerkt)
    const idx = Math.random() < 0.5 ? 0 : 1;
    const played = actor.hand.splice(idx, 1)[0];
    if (played) room.discard.push(played);
    log(room, `${actor.name} speelt ${played?.name ?? 'een kaart'} (bot)`);
